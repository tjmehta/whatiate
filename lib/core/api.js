...
var users = (function(){
  ...
  Users.prototype.getUserById = function(userId, callback)
  { 
	callback(db.users.findOne(userId));
  }
	return new Users();
})();
	...
	Remember.prototype.getAndStoreToken = function(userId, frob, callback)	{
		...
		function(error, response, body)
		{
			...
			if(!error && response.statusCode == 200){
				...
				db.users.update({_id: userId}, [], $set:{milkToken: body.token}, {safe: true}, function(error, user) {
					if (error) {
						callback(error);
						return;
					}														
